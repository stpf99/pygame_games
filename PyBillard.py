...
class Cue:
    ...
    def start_charging(self):
        if self.ball.active and not self.ball.is_moving():
            self.charging = True
            self.power = 0  # Początkowa moc ustawiona na 0

    def adjust_power(self, direction):
        if self.charging:
            self.power = max(0, min(self.power + direction, self.max_power))
...
class Game:
    ...
    def setup_balls(self):
        ...
        colors = [
            YELLOW, BLUE, RED, PURPLE, ORANGE, GREEN, MAROON, BLACK,  # 1-8
            YELLOW, BLUE, RED, PURPLE, ORANGE, GREEN, MAROON         # 9-15
        ]
        ...
        ball_positions = [
            ...
            (1, -1), (1, 1),           # 2, 3
            (2, -2), (2, 0), (2, 2),  # 4, 5, 6
            (3, -3), (3, -1), (3, 1), (3, 3),  # 7, 8, 9, 10
            (4, -4), (4, -2), (4, 0), (4, 2), (4, 4)  # 11, 12, 13, 14, 15
        ]
    ...
    def update(self):
        ...
        if all_stopped:
            ...
            if potted_balls or first_hit:
                ...
                if first_hit and current_type:
                    if (current_type == "solid" and first_hit > 8) or (current_type == "striped" and first_hit <= 7):
                        foul = True
                if current_type == "solid" and any(b.number > 8 for b in potted_balls if b.number != 8):
                    foul = True
                ...
                if foul or (not any(b.number != 0 and b.number != 8 for b in potted_balls)):
    ...
    def draw(self, screen):
        ...
        remaining1 = sum(1 for b in self.balls if b.active and b.number != 0 and
                        ((b.number <= 7 and self.player1_type == "solid") or
                         (b.number > 8 and self.player1_type == "striped")))
        if self.player_turn == 1 and self.blink_state:
            p1_text = font.render(f"Gracz 1: {self.player1_potted}/{7 if self.player1_type else 0}", True, RED)
        else:
            p1_text = font.render(f"Gracz 1: {self.player1_potted}/{7 if self.player1_type else 0}", True, BLACK)
        screen.blit(p1_text, (20, 20))
        if self.player1_type:
            type_text = font.render(f"Typ: {self.player1_type} ({remaining1} pozostało)", True, BLACK)
            screen.blit(type_text, (20, 50))

        remaining2 = sum(1 for b in self.balls if b.active and b.number != 0 and
                        ((b.number <= 7 and self.player2_type == "solid") or
                         (b.number > 8 and self.player2_type == "striped")))
        if self.player_turn == 2 and self.blink_state:
            p2_text = font.render(f"Gracz 2: {self.player2_potted}/{7 if self.player2_type else 0}", True, RED)
        else:
            p2_text = font.render(f"Gracz 2: {self.player2_potted}/{7 if self.player2_type else 0}", True, BLACK)
        p2_rect = p2_text.get_rect(topright=(WIDTH - 20, 20))
        screen.blit(p2_text, p2_rect)
        if self.player2_type:
            type_text = font.render(f"Typ: {self.player2_type} ({remaining2} pozostało)", True, BLACK)
            type_rect = type_text.get_rect(topright=(WIDTH - 20, 50))
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if game.cue.charging:
                    # Przewijanie w górę zwiększa moc, w dół zmniejsza
                    game.cue.adjust_power(event.y * 1.0)  # event.y: 1 dla góry, -1 dla dołu
